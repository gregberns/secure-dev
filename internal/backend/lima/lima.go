...
func (b *limaBackend) generateLimaYAML(name string, cfg backend.VMConfig) (string, error) {
	...
	// Images - resolve short name to per-architecture URLs
...
// imageEntry holds a per-architecture image URL for Lima YAML generation.
// REQ-003-024: Multi-architecture base image support
type imageEntry struct {
	Arch     string // "aarch64" or "x86_64" (Lima arch names)
	Location string
}

// imageMap maps base image short names to their per-architecture URLs.
// REQ-003-024: Multi-architecture base image support
var imageMap = map[string][]imageEntry{
	"ubuntu:24.04": {
		{Arch: "aarch64", Location: "https://cloud-images.ubuntu.com/releases/24.04/release/ubuntu-24.04-server-cloudimg-arm64.img"},
		{Arch: "x86_64", Location: "https://cloud-images.ubuntu.com/releases/24.04/release/ubuntu-24.04-server-cloudimg-amd64.img"},
	},
	"ubuntu:22.04": {
		{Arch: "aarch64", Location: "https://cloud-images.ubuntu.com/releases/22.04/release/ubuntu-22.04-server-cloudimg-arm64.img"},
		{Arch: "x86_64", Location: "https://cloud-images.ubuntu.com/releases/22.04/release/ubuntu-22.04-server-cloudimg-amd64.img"},
	},
	"debian:12": {
		{Arch: "aarch64", Location: "https://cloud.debian.org/images/cloud/bookworm/daily/latest/arm64/disk.qcow2"},
		{Arch: "x86_64", Location: "https://cloud.debian.org/images/cloud/bookworm/latest/debian-12-generic-amd64.qcow2"},
	},
}

// resolveBaseImage resolves a short name to Lima image entries for all supported architectures.
// REQ-003-024: Multi-architecture base image support
func resolveBaseImage(name string) ([]imageEntry, error) {
	if entries, ok := imageMap[name]; ok {
		return entries, nil
	}

	// If it looks like a URL, pass through as a single entry without arch constraint
