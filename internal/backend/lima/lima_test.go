...
// TestLimaYAML_BaseImageResolution verifies short names resolve to URLs for both architectures.
...
func TestLimaYAML_BaseImageResolution(t *testing.T) {
	...
	tests := []struct {
		name        string
		baseImage   string
		expectURLs  []string // URLs that must appear in generated YAML
		expectError bool
	}{
		{
			name:      "ubuntu-24-04",
		...
		},
		{
			...
			baseImage: "debian:12",
			expectURLs: []string{
				"https://cloud.debian.org/images/cloud/bookworm/daily/latest/arm64/disk.qcow2",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, url := range tt.expectURLs {
				if !strings.Contains(yaml, url) {
					t.Errorf("YAML missing expected URL %q", url)
				}
			}
		})
	}
...
// TestResolveBaseImage_ArchEntries verifies resolveBaseImage returns entries for both architectures.
// REQ-003-024: Multi-architecture base image support
func TestResolveBaseImage_ArchEntries(t *testing.T) {
	tests := []struct {
		name       string
		baseImage  string
		wantArches []string
		...
		{
			name:       "ubuntu-24-04-both-arches",
			baseImage:  "ubuntu:24.04",
			wantArches: []string{"aarch64", "x86_64"},
		},
		{
			name:       "ubuntu-22-04-both-arches",
			baseImage:  "ubuntu:22.04",
			wantArches: []string{"aarch64", "x86_64"},
		},
		{
			name:       "debian-12-both-arches",
			baseImage:  "debian:12",
			wantArches: []string{"aarch64", "x86_64"},
		},
		{
			name:       "full-url-no-arch",
			baseImage:  "https://example.com/custom.img",
			wantArches: []string{""},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			entries, err := resolveBaseImage(tt.baseImage)
			if tt.wantError {
				if err == nil {
			...
			if len(entries) != len(tt.wantArches) {
				t.Fatalf("got %d entries, want %d", len(entries), len(tt.wantArches))
			}
			for i, wantArch := range tt.wantArches {
				if entries[i].Arch != wantArch {
					t.Errorf("entry[%d].Arch = %q, want %q", i, entries[i].Arch, wantArch)
				}
				if entries[i].Location == "" {
					t.Errorf("entry[%d].Location is empty", i)
				}
			}
		})
	}
...
// TestLimaYAML_ArchFieldsInImages verifies generated YAML includes arch fields for known images.
// REQ-003-024: Multi-architecture base image support
func TestLimaYAML_ArchFieldsInImages(t *testing.T) {
	b := New()
	...
	// Verify both arch entries appear
	if !strings.Contains(yaml, "arch: \"aarch64\"") {
		t.Error("YAML missing arch: \"aarch64\" field")
	}
	if !strings.Contains(yaml, "arch: \"x86_64\"") {
		t.Error("YAML missing arch: \"x86_64\" field")
	}

	// Verify custom URL has no arch field
	cfg.BaseImage = "https://example.com/custom.img"
	yaml, err = b.(*limaBackend).generateLimaYAML("testvm", cfg)
	if err != nil {
		t.Fatalf("generateLimaYAML failed for custom URL: %v", err)
	}
	if strings.Contains(yaml, "arch:") {
		t.Error("custom URL should not have arch field in YAML")
	}
}
